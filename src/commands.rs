...
pub mod transmission_lineage {
    ...
    impl LineageFinder {
        fn new(key: String, value: AnnotationValue, ignore_taxa: HashSet<String>, cutoff: f64, lag: f64) -> Self {
            LineageFinder { lineages: vec![], key, value, ignore_taxa, cutoff, lag }
        }
        fn clear(&mut self) {
            self.lineages = vec![];
        }
            ...
            if let Some(mut parent) = tree.get_parent(node) {
                ...
                if annotation == &self.value {
                    if let Some(li) = lineage_index { // parent was in this lineage
                        ...
                        } else { // if it respects the lag
                            let current_index = if self.respects_lag(tree, node, 0.0) { lineage_index } else { None };
                            for child in tree.get_children(node) {
                                self.find_lineages(tree, child, current_index);
                            }
                        };
                    } else if tree.get_height(node).expect("nodes should have heights") >= self.cutoff &&  self.respects_lag(tree, node, 0.0) {
                        // new lineage
                ...
                //At the root
                let annotation = tree.get_annotation(node, &self.key).unwrap();
                if annotation == &self.value && self.respects_lag(tree, node, 0.0) {
                    let id = self.lineages.len();
                    let new_lineage = TransmissionLineage {
                        taxa: vec![],
                        tmrca: tree.get_height(node).unwrap(),
        ...
        fn respects_lag(&self, tree: &MutableTree, node: usize, current_lag: f64) -> bool {
            let mut respects = false;
            if self.lag == f64::INFINITY {
                respects = true;
            } else if tree.get_annotation(node, &self.key).expect("node should annotation") != &self.value {
                respects = false;
            } else if current_lag> self.lag{
                respects =false;
                ...
                respects = true;
                    ...
                    for child in tree.get_children(node) {
                        ...
                        respects = respects || self.respects_lag(tree, child, l);
                        if respects {
                            break;
                        }
                    }
    ...
    pub fn run<R: std::io::Read, T: TreeImporter<R>>(mut trees: T, ignore_taxa: Option<path::PathBuf>, key: String, value: String, taxa_flag: bool, origin: Option<f64>, cutoff: Option<f64>, lag: Option<f64>) -> Result<(), Box<dyn Error>> {
        ...
        let mut lineage_finder = LineageFinder::new(key, AnnotationValue::Discrete(value), ignore, most_recent_intro, max_lag);
        while trees.has_tree() {
            let mut tree = trees.read_next_tree()?;
    ...
    mod tests {
        ...
        fn find_lineages() {
            ...
            tree.calc_node_heights();
            let mut lf = LineageFinder::new("location".to_string(), AnnotationValue::Discrete("UK".to_string()), HashSet::new(), f64::NEG_INFINITY,f64::INFINITY);
        ...
        fn find_lineages_lag() {
            ...
            //TODO work this into Lineage Finder
            tree.calc_node_heights();
            let mut lf = LineageFinder::new("location".to_string(), AnnotationValue::Discrete("UK".to_string()), HashSet::new(), f64::NEG_INFINITY,0.05);
        ...
        fn find_2lineages() {
            ...
            //TODO work this into Lineage Finder
            tree.calc_node_heights();
            let mut lf = LineageFinder::new("location".to_string(), AnnotationValue::Discrete("US".to_string()), HashSet::new(), f64::NEG_INFINITY,1.0);
