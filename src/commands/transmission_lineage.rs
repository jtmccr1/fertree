...
impl LineageFinder {
                ...
                if let Some(li) = lineage_index { // parent was in this lineage
                    if tree.is_external(node) {
                        ...
                        if self.will_be_sampled_before_lag(tree, node, 0.0) && !self.has_been_sampled_within_lag(tree, node, 0.0) {
                            ...
                            let current_index = if self.will_be_sampled_before_lag(tree, node, 0.0) { lineage_index } else { None };
                            for child in tree.get_children(node) {
                                self.find_lineages(tree, child, current_index);
                            }
                        }
                    };
                } else if tree.get_height(node).expect("nodes should have heights") >= self.cutoff && self.will_be_sampled_before_lag(tree, node, 0.0) {
                    // new lineage
    ...
    fn has_been_sampled_within_lag(&mut self, tree: &MutableTree, mut node: usize, current_lag: f64) -> bool {
        let mut respects = false;
        if self.lag == f64::INFINITY {
            respects = true;
        }
...
mod tests {
    ...
    fn find_lineages_internal_lag() {
        ...
        //TODO work this into Lineage Finder
        tree.calc_node_heights();
        let mut lf = LineageFinder::new("location".to_string(), AnnotationValue::Discrete("UK".to_string()), HashSet::new(), f64::NEG_INFINITY, 1.0);
